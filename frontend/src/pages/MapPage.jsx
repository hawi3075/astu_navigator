...
const createPlaceIcon = (category, isTarget) => {
  ...
  // Reduced size from 38px to 28px for a cleaner look
  return L.divIcon({
    ...
    `,
    iconSize: [28, 28],
    iconAnchor: [14, 28], // Pointy end of the pin
    popupAnchor: [0, -28]
  });
};
...
export default function MapPage({ onNavigate, userEmail }) {
  const [messages, setMessages] = useState([{ text: "Welcome to ASTU! Icons are now minimized for a better view.", isBot: true }]);
  ...
  const handleSaveLocation = async (locationName) => {
    ...
    try {
      await axios.post('http://localhost:8000/api/save-location', { user_email: userEmail, location_name: locationName });
      ...
      <div className="flex-1 flex overflow-hidden p-6 gap-6 mt-1 mb-2"> 
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-50/50">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.isBot ? 'justify-start' : 'justify-end'}`}>
                <div className={`p-4 rounded-2xl text-sm ${m.isBot ? 'bg-white text-slate-700 shadow-sm' : 'bg-blue-600 text-white shadow-lg'} max-w-[85%]`}>
          ...
          <div className="p-4 bg-white border-t">
            <div className="relative flex items-center">
              <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()}
                className="w-full bg-slate-100 rounded-xl p-4 pr-12 outline-none focus:ring-2 focus:ring-blue-500 font-medium" placeholder="Search..." />
        ...
        <div className="flex-1 rounded-[2.5rem] overflow-hidden relative border-8 border-white bg-white shadow-2xl">
          <MapContainer center={ASTU_CENTER} zoom={16} className="h-full w-full">
            ...
            {/* RENDER MINI ICONS FROM DATABASE */}
            {dbLocations.map((loc) => (
              <Marker 
                key={loc._id} 
          ...
          <div className="absolute bottom-6 left-6 z-[1000] bg-white/95 backdrop-blur-md p-4 rounded-2xl shadow-xl flex items-center gap-3">
             ...
             <div>
               <p className="text-[9px] font-black text-blue-600 uppercase leading-none">Navigation Active</p>
